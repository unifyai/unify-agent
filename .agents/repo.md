...
- The runtime's home is `UNIFY_HOME` (default `~/.unify`): `store.sqlite`, `embeddings.sqlite` (the skill search embeddings cache), `workspace/` (the actor's working directory, attachments and downloads) and `logs/`.
